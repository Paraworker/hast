...
impl Acceptor {
        ...
        I: IntoIterator<Item = usize>,
    {
        ...
        for index in range {
            match Self::bind(&xdg, format!("{basename}-{index}")) {
                ...
                Err(AcceptorError::LockInUse) => continue,
        ...
        S: AsRef<str> + Into<String>,
    {
        ...
        let _lock = Self::try_lock_file(&lock_path)?;
    ...
    fn try_lock_file(path: &Path) -> Result<File, AcceptorError> {
        ...
        loop {
            ...
            if let Err(_) = flock(&lock, FlockOperation::NonBlockingLockExclusive) {
                return Err(AcceptorError::LockInUse);
            }
